    ...
    function maybeComplete() {
        if (error) {
            ...
            for (const id in json) {
                const {width, height, x, y, sdf, anchorX, anchorY, pixelRatio, stretchX, stretchY, content} = json[id];
                const data = new RGBAImage({width, height});
                RGBAImage.copy(imageData, data, {x, y}, {x: 0, y: 0}, {width, height});
                result[id] = {data, pixelRatio, sdf, anchorX, anchorY, stretchX, stretchY, content};
            }
